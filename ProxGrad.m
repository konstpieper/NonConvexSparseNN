function u = ProxGrad(p, Sred, ref, alpha, phi, u0, NQ)
%{ 
%% Solve the problem: 
...
l1 = @(u) alpha*sum(computeNorm(u, NQ));

% nonconvex part (only for scalar sparsity)
Dphima = @(u) (phi.dphi(abs(u)) - 1) .* sign(u);

% prox stuff
P = @(sigma, q) computeProx(q, sigma*alpha, NQ);
G = @(u) alpha*Dphima(u) + Sred'*F.dF(Sred*u - ref);
...
while ~converged && iter < 66666 && iterls < 150000
    ...
    jpred = j + (unew - u)'*Gu + 1/(2*sigma)*norm(unew - u)^2 - l1(u) + l1(unew);
    jnew = obj(unew);
...
%     end
    ...
    if mod(iter, 100) == 0
      fprintf('\t\tprox: %i, supp: %i, desc: %1.1e, res: %1.1e, stepsize: %1.1e\n', ...
            iter, nnz(computeNorm(u, NQ)), desc, normG, sigma);
    end 
    iter = iter + 1;

    converged = (normG < max(1e-1*alpha, 1e-5*normG0));

end
fprintf('\tprox: %i, supp: %i, desc: %1.1e, res: %1.1e, stepsize: %1.1e\n', ...
        iter, nnz(computeNorm(u, NQ)), j0-j, normG, sigma);
