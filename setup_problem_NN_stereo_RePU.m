function p = setup_problem_NN_stereo_RePU(power)
#{
...
%% the stereographic projection x = stereo(a,b). 
  ...
  %% kernel = max(0, y^p)
  absy_p = max(0, y).^p.power;

  k = absy_p;
  dk = p.power * max(0, y).^(p.power-1) .* dydx;
...
while ng > 1e2*TOL && iter < 10
  ...
  [xmax, ~, cvg, out] = fminunc(j, x0(:), opts);
  ...
  u_pp = p.postprocess(p, struct('x', xmax, 'u', ones(1,length(xmax))), 1e3*TOL);
...
%figure(667)
%p.plot_adjoint(p, p.u_zero, y, 0);
%hold on;
%unstereo = @(x) atan2(2*x./(1+x.^2), (1-x.^2)./(1+x.^2));
%plot(unstereo(xmax), Ks(p, xmax, p.xhat, y), 'xk');
%hold off;
%drawnow;
